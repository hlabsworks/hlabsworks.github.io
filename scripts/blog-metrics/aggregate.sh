...
# 日次/月次/5分プロファイルの「集計値のみ」を取得し、build_daily.py -> layer_model.py ->
# bill_model.py の順にオーケストレーションして Hugo の data ディレクトリ
...
# --- build_daily.py（daily.json/monthly.json/meta.json を生成） ---
...
# --- bill_model.py（必ず layer_model.py の後に実行する順序を守る） ---
...
echo "aggregate.sh: done (profile rows: ${PROFILE_ROW_COUNT})"
