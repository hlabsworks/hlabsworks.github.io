...
契約中の新電力プランの推定請求額を再現し、太陽光なし(L0)反実仮想との差分から
節約額（FIT実態 / 卒FIT換算）を算出する。

入力:
  - scripts/blog-metrics/tariff.json      （請求明細PDFベースの実績単価データ、手動更新。
    居住地域の推定を防ぐため小売事業者名・プラン名・供給エリアは非公開値にしている）
  - data/metrics/daily.json               （aggregate.sh の生成物。暦日ごとの実測 kWh）
  - data/metrics/official_sell.json（任意）（import_official_sell.py の生成物。電力会社の
    公式メーター売電実績。無い場合はセンサー(power_history)由来の
    sell_kwh にフォールバックする）
  - data/metrics/official_buy.json（任意）（import_official_buy.py の生成物。契約中の
    新電力の請求明細PDFの実使用量(usage_kwh)。無い場合はセンサー(power_history)由来の
    buy_kwh にフォールバックする）
...
設計方針:
  ...
  - 売電収入（FIT実態）は official_sell.json に対応する精算月があればそれを優先採用する
    （電力会社の実支払額のため、センサー推定より正確）。算定期間が
    daily.json 側の請求期間と一致しない場合は信用せずセンサー値にフォールバックする。
    採用した売電量の出典は各月の "sell_source" ("official_meter"|"sensor") に記録し、
  ...
  - 円換算は、請求明細PDF13か月分（private の energy-archive、2026-09-05抽出）との
    突合で確定した実際の端数処理方式（VERIFIED）に従う:
...
def _round_yen(value: float) -> int:
    """円単位に四捨五入する（ROUND_HALF_UP）。売電収入（電力会社の購入実績・センサー推定）の
...
def build_month_record(
    ...
    if official_sell is not None:
        ...
        sell_source = "official_meter"
        # 電力会社が実際に支払った金額（購入実績お知らせサービス）をそのまま採用する。
