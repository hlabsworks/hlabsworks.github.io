...
grid_sell/purchase_monthly.json から、公開しても問題ない集計値のみを抽出して
...
出力しない。抽出するのは以下の集計値のみ:
  ...
  - sell_revenue_yen（電力会社から実際に支払われた売電収入額。
    全月で purchase_yen == 16円 × purchase_kwh_from_yen が厳密一致することを確認済み
    — VERIFIED 2026-09-05、purchase_monthly.json の12ヶ月分で検算）。
...
# 2026-09-23 居住地域の推定を防ぐため既定パスのサブディレクトリ名を汎用名に変更した。
# private の energy-archive 側のディレクトリ名もこれに合わせて改名するか、
# --source で明示的に指定すること。
DEFAULT_SOURCE_PATH = Path.home() / "Develop" / "energy-archive" / "grid_sell" / "purchase_monthly.json"
