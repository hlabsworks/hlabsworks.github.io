...
日次ゲート（DDR §3・§0既知のノイズ、オーナー承認機能・2026-09-06、閾値緩和2026-09-20）:
nichicon_realtime_historyは300秒瞬時値のためジッタで5分バケットを1個落とすのは常態。
resolve_day_buckets()が、1日あたりチャネル（nichicon_socを含む）ごとの欠落スロット数が
INTERPOLATION_MAX_GAP(6)個以下なら前後の実測値で線形補間して埋める（先頭/末尾の欠落は
最近傍値）。いずれかのチャネルで7個以上の欠落・バケット行数超過・想定外時刻のバケット混入・
bucket_at重複・プロファイル自体が無い場合は当該日を不採用のまま（period_incomplete/
profile_missing）にし、捏造しない。
補間結果は各レコードの interpolated_buckets（補間したチャネル値の合計数）と
interpolated_slots（補間が発生した時刻スロットの実数、QA再レビュー2026-09-06 #6で追加）に
記録する。

請求期間の使用可能日が90%未満の月はL0〜L2をunavailableにする（オーナー決定2026-09-20）。
90%以上100%未満（usableな日が連続でなくてよい）の月は、usableな日のみで求めたbuy_kwh/
sell_kwhを「期間日数／usable日数」で月額換算(scale_factor)してからcompute_billに渡し、
各層(L0〜L2)に estimation:"scaled" / days_usable / days_total / scale_factor /
excluded_dates（除外日と理由コード）を付与する。100%usableな月は estimation:"full"
（従来どおりスケールなし）。L3（実測）はこのスケーリングと無関係（請求実績・センサー日次を
そのまま使う）ため、scaledの月ではL0〜L2が日数比換算の推定値、L3が実請求という非対称が
生じる（_note・開示表で開示）。暫定単価（未確定請求月への直近確定月フォールバック）は
確定月表示には使わない（build_in_progressのみで許容）。
...
# オーナー決定2026-09-20: usable日が請求期間の90%未満ならL0〜L2はunavailable（按分・満月
# 換算しない。部分月をあたかも満月であるかのように compute_bill すると L0 が系統的に過小
# 評価される。実例: 1日欠測で3.2%過小 = ¥33,173 vs ¥34,258）。90%以上100%未満は
# usableな日（連続でなくてよい）のみで求めたbuy_kwh/sell_kwhを「期間日数／usable日数」で
# 換算(scale_factor)してから compute_bill に渡す（build_month_layers参照）。
...
# DDR §0既知のノイズ対策（オーナー承認機能・2026-09-06、閾値緩和2026-09-20）:
# nichicon_realtime_history は300秒瞬時値のため、ジッタで5分バケットを1個落とすのは常態
# （DDRの言う「隣接サンプル線形補間」対策そのもの）。1日あたりチャネルごとの欠落が
# INTERPOLATION_MAX_GAP 個以下なら前後の有効値で線形補間して埋める（先頭/末尾の欠落は
# 最近傍値）。それを超える場合は従来どおり当日を不採用にし、捏造しない。
# 2026-09-20オーナー決定: Pi Wi-Fi断（1日4〜6スロット欠落）でin_progressが恒久的に止まる
# 実害が出たため 3→6 に緩和（依然として1チャネルあたり連続でなく合計欠落数での判定）。
INTERPOLATION_MAX_GAP = 6
...
def resolve_day_buckets(buckets: list[Bucket] | None, d: date) -> DayResolution:
    ...
    INTERPOLATION_MAX_GAP(6)個以下なら、前後の有効値で線形補間して288バケット全てを埋める
    （先頭/末尾の欠落は最近傍値）。いずれかのチャネルで欠落が7個以上、バケット行数が288を
...
def _simulate_l2_series_detailed(
    ...
    日付ごとの(buy_kwh, sell_kwh)・最終soc_pct・最大逆潮流(max_export_w)を返す
    （オーナー承認機能・2026-09-06: 日次4層系列／2026-09-20: 90%スケール運用で確定月・
    in_progressの主計算にも使うため戻り値にsoc/max_exportを追加）。

    バケット列の5分間隔の連続性が途切れる箇所（欠測日等のギャップ、usableな日だけを
    連結した非連続な日集合を含む）では実測nichicon_socに再アンカーし、不明な期間の
    充放電を捏造しない（ドリフトを蓄積させない）。DDR §2.4の「期間開始時刻の実測socで
    初期化」をギャップ発生のたびにも適用したもの。
    """
    ...
    return daily_kwh, soc_pct, max_export


def simulate_l2_series(
    ordered_buckets: list[Bucket], pv_ac_efficiency: float, initial_soc_pct: float
) -> dict[str, tuple[float, float]]:
    """日付ごとの(buy_kwh, sell_kwh)のみを返す薄いラッパー（build_daily_layers用）。
    最終soc_pct/max_export_wが必要な呼び出し元は _simulate_l2_series_detailed を直接使う
    （二重実装しない）。"""
    daily_kwh, _soc_end_pct, _max_export = _simulate_l2_series_detailed(
        ordered_buckets, pv_ac_efficiency, initial_soc_pct
    )
    return daily_kwh
...
def _day_exclusion_entry(d_str: str, day_buckets: list[Bucket] | None) -> dict:
    ...
    bucket_at重複・行数異常等、resolve_day_buckets が None を返す理由の詳細は問わない）を
    reason_code/reason_label のみで区別する（内部ファイル名等を含むreason_detailは持たせない。
    開示表は日付のみを表示しreason_labelをホバーで見せる想定）。"""
    if not day_buckets:
        return {
            "date": d_str,
            "reason_code": bill_model.REASON_CODE_PROFILE_MISSING,
            "reason_label": "5分プロファイル未取得",
        }
    return {
        "date": d_str,
        "reason_code": bill_model.REASON_CODE_PERIOD_INCOMPLETE,
        "reason_label": "5分プロファイル欠落",
    }
...
def build_month_layers(
    ...
    # --- L0/L1/L2（推定）: usable日が請求期間の90%未満ならunavailable、90%以上なら
    # usableな日（連続でなくてよい）のみで按分・月額換算する（オーナー決定2026-09-20、
    # MONTH_USABLE_FRACTION_THRESHOLD参照） ---
    days_in_period = [start + timedelta(days=i) for i in range(total_days)]
    excluded_dates: list[dict] = []
    usable_day_buckets: dict[str, list[Bucket]] = {}
    ...
    for d in days_in_period:
        ...
        if resolution.buckets is None:
            excluded_dates.append(_day_exclusion_entry(d_str, day_buckets))
        else:
            usable_day_buckets[d_str] = resolution.buckets
            month_interpolated_buckets += resolution.interpolated_values
            month_interpolated_slots += resolution.interpolated_slots

    days_usable = len(usable_day_buckets)
    coverage = (days_usable / total_days) if total_days else 0.0

    if coverage < MONTH_USABLE_FRACTION_THRESHOLD:
        ...
        for d_str in sorted(usable_day_buckets):
            ordered_buckets.extend(usable_day_buckets[d_str])
        ordered_buckets.sort(key=lambda b: b.bucket_at)
        resampled5 = resample_buckets(ordered_buckets, BUCKET_MINUTES)
        ...
        # usableな日が非連続な場合でもギャップで実測socに再アンカーするため、単発ではなく
        # 系列版(_simulate_l2_series_detailed)を使う（DDR「L2のSOCは連続しない箇所では
        # 実測SOCに再アンカーする」、build_in_progressと同じ扱い）。
        l2_daily, soc_end_pct, l2_max_export = _simulate_l2_series_detailed(
            ordered_buckets, pv_ac_efficiency=1.00, initial_soc_pct=soc_start_pct
        )
        l2_totals = LayerTotals(
        ...
        estimation = "full" if is_full else "scaled"
        scale_factor = (total_days / days_usable) if days_usable else 0.0
        common_extra = {
        ...
        eco_warning = eco_balance_warning(ordered_buckets)
        if eco_warning:
        ...
        try:
            l0_layer = layer_dict(
                "estimated", True, l0_totals.buy_kwh * scale_factor, l0_totals.sell_kwh * scale_factor,
                tariff, billing_month, sell_fit, sell_post_fit, extra=dict(common_extra),
            )
            l1_layer = layer_dict(
                "estimated", True, l1_totals.buy_kwh * scale_factor, l1_totals.sell_kwh * scale_factor,
                tariff, billing_month, sell_fit, sell_post_fit, extra=dict(common_extra),
            )
            l2_layer = layer_dict(
                "estimated", True, l2_totals.buy_kwh * scale_factor, l2_totals.sell_kwh * scale_factor,
                tariff, billing_month, sell_fit, sell_post_fit,
                extra={
                    **common_extra,
            ...
            # Jensenバイアスでbuy/sellが両方かさ上げされる）。scaled月はここもscale_factorを
            # 適用してから請求額換算する（ヘッドライン値と同じ土俵の範囲にする）。単発の
            # simulate_l2（系列版のギャップ再アンカーを使わない簡略版）のままで十分な精度の
            # 近似レンジのため、ここだけは既存どおり単純化を維持する。
            l1_costs = []
            l2_costs = []
            for bucket_minutes in (5, 15, 30):
                ...
                for eff in (1.00, 0.95):
                    ...
                    l1_bill = bill_model.compute_bill(tariff, l1_t.buy_kwh * scale_factor, billing_month)
                    l1_costs.append(l1_bill.total_yen - _round_yen(l1_t.sell_kwh * scale_factor * sell_fit))
                    l2_t, _ = simulate_l2(resampled, pv_ac_efficiency=eff, soc_start_pct=soc_start_pct)
                    l2_bill = bill_model.compute_bill(tariff, l2_t.buy_kwh * scale_factor, billing_month)
                    l2_costs.append(l2_bill.total_yen - _round_yen(l2_t.sell_kwh * scale_factor * sell_fit))
            uncertainty = {
            ...
            # 暫定単価に頼らずunavailableにする（既存のL3側tariff_missing処理と同じ扱い）。
            tariff_reason = bill_model.reason(
...
def build_daily_load(profile_by_date: dict[str, list[Bucket]]) -> list[dict]:
    ...
    （捏造しない）。1日INTERPOLATION_MAX_GAPバケットまでの欠落は線形補間して埋める
    （オーナー承認機能・2026-09-06、閾値緩和2026-09-20）。"""
    days = []
...
def build_daily_layers(tariff: dict, daily_by_date: dict, profile_by_date: dict[str, list[Bucket]]) -> list[dict]:
    ...
    # 1日INTERPOLATION_MAX_GAPバケットまでの欠落は線形補間して埋める（オーナー承認機能・2026-09-06）。
...
def build_in_progress(
    ...
    異なり、期間の全日が揃うのを待たず開始日〜period_end_actualまでのデータで compute_bill
    する（段階制・容量拠出金込み、通常どおり）。単価が未確定なら直近確定月の単価を暫定適用し
    tariff_provisional / tariff_source_month で明示する（確定表示にのみ捏造禁止方針を適用し、
    月途中集計は明示ラベル付きで暫定単価を許容 — オーナー承認済み）。

    2026-09-20オーナー決定（S_1深放電インシデント対策・Pi Wi-Fi断でin_progressが恒久停止した
    実害への対応）: 従来は「startから連続してusableな日」で打ち切っていたため、窓の途中に
    1日でも欠測が挟まると、それ以降にusableな日があっても一切反映されなかった（9/5・9/6の
    DELTAテレメトリ欠落、9/12〜9/14のWi-Fi断で2026-10請求月が永久に進まなくなった）。
    修正後は period_end_actual = min(end, today−1日)（today自体は部分行のため常に除外、
    QA再レビュー #2 を維持）に固定し、[start, period_end_actual] の範囲でusableな日
    （プロファイルがusable かつ daily.json に buy_kwh がある日、連続でなくてよい）だけを
    集めて4層すべてを同じ日集合で合算する（既存の「4層は必ず同一日集合」不変条件を維持）。
    days_covered はusable日数、days_elapsed は start〜period_end_actual の暦日数
    （usable日数とは独立。両者の差が「今回除外された日数」）、excluded_dates は
    [start, period_end_actual] のうちusableでない日と理由コード。
    L2のSOCはusable日を時系列順に連結して_simulate_l2_series_detailedでシミュレートし、
    非連続な箇所（欠測日をまたぐ箇所）では実測nichicon_socに再アンカーする（DDR §2.4を
    ギャップ発生のたびに適用。build_month_layersの90%スケール運用と同じ扱い）。
    [start, period_end_actual] にusableな日が1日も無ければ None を返す。
    """
    meter_read_day = tariff["meter_read_day"]
    ...
    period_end_actual = min(end, today - timedelta(days=1))
    if period_end_actual < start:
    ...
    # [start, period_end_actual] の範囲で、profileがusableな日を先に全て解決しておく。
    resolved_by_date: dict[str, list[Bucket]] = {}
    interpolated_values_by_date: dict[str, int] = {}
    interpolated_slots_by_date: dict[str, int] = {}
    for d in bill_model._daterange(start, period_end_actual):
        d_str = d.isoformat()
        day_buckets = profile_by_date.get(d_str)
        resolution = resolve_day_buckets(day_buckets, d)
        if resolution.buckets is not None:
    ...
    # usable_dates = プロファイルがusable かつ daily.json にbuy_kwhがある日の共通集合
    # （4層は必ず同一日集合、という既存不変条件をここで確定する）。連続でなくてよい。
    usable_dates: list[str] = []
    excluded_dates: list[dict] = []
    for d in bill_model._daterange(start, period_end_actual):
        d_str = d.isoformat()
        profile_ok = d_str in resolved_by_date
        daily_ok = (daily_by_date.get(d_str) or {}).get("buy_kwh") is not None
        if profile_ok and daily_ok:
            usable_dates.append(d_str)
        elif not profile_ok:
            excluded_dates.append(_day_exclusion_entry(d_str, profile_by_date.get(d_str)))
        else:
            excluded_dates.append({
                "date": d_str,
                "reason_code": bill_model.REASON_CODE_DAILY_MISSING,
                "reason_label": "計測データ欠測",
            })

    days_covered = len(usable_dates)
    days_elapsed = (period_end_actual - start).days + 1
    if days_covered == 0:
        return None

    # --- L3: センサー(daily.json)、usable_datesのみ合算（L0〜L2と同じ日集合） ---
    l3_buy_kwh = sum(daily_by_date[d]["buy_kwh"] for d in usable_dates)
    l3_sell_kwh = sum((daily_by_date[d].get("sell_kwh") or 0.0) for d in usable_dates)
    try:
    ...
    # --- L0/L1/L2: usable_dates（L3と同じ日集合）のバケットを時系列順に連結して合算 ---
    period_interpolated_values = sum(interpolated_values_by_date[d] for d in usable_dates)
    period_interpolated_slots = sum(interpolated_slots_by_date[d] for d in usable_dates)
    ordered_buckets: list[Bucket] = []
    for d_str in usable_dates:
        ordered_buckets.extend(resolved_by_date[d_str])
    ordered_buckets.sort(key=lambda b: b.bucket_at)
    ...
    soc_start_pct = ordered_buckets[0].nichicon_soc if ordered_buckets[0].nichicon_soc is not None else 0.0
    l2_daily, soc_end_pct, l2_max_export = _simulate_l2_series_detailed(
        ordered_buckets, pv_ac_efficiency=1.0, initial_soc_pct=soc_start_pct
    )
    l2_totals = LayerTotals(
        buy_kwh=sum(v[0] for v in l2_daily.values()),
        sell_kwh=sum(v[1] for v in l2_daily.values()),
...
def _build_cumulative(months: list[dict]) -> dict:
    ...
    オーナー決定2026-09-20: usable日が90〜100%の「scaled」月も available:true のため累計に
    含める（100%の「full」月と同列に合算する）。ただしscaled月が1つでも混じっている場合は
    includes_scaled_months:true を立て、L0〜L2が日数比換算の推定値である月を含むことを
    読者に開示する（L3は各月とも実測のまま、非対称性の開示）。"""
