...
class BuildMonthLayersTest(unittest.TestCase):
    ...
    def test_l3_sell_prefers_official_value(self):
        ...
        self.assertEqual(record["layers"]["L3"]["sell_source"], "official_meter")
...
class BuildCumulativeTest(unittest.TestCase):
    ...
    def _month(self, billing_month: str, available: dict, estimation: dict | None = None) -> dict:
        layers = {}
        for key in ("L0", "L1", "L2", "L3"):
            if available.get(key, True):
                layers[key] = {"available": True, "net_cost_fit_yen": {"L0": 10000, "L1": 7000, "L2": 4000, "L3": 3000}[key]}
                if estimation and key in estimation:
                    layers[key]["estimation"] = estimation[key]
            else:
    ...
    def test_includes_scaled_months_false_when_all_full(self):
        months = [self._month("2026-08", available={}, estimation={"L0": "full", "L1": "full", "L2": "full"})]
        cumulative = lm._build_cumulative(months)
        self.assertFalse(cumulative["includes_scaled_months"])

    def test_includes_scaled_months_true_when_any_layer_scaled(self):
        months = [
            self._month("2026-08", available={}, estimation={"L0": "full", "L1": "full", "L2": "full"}),
            self._month("2026-09", available={}, estimation={"L0": "scaled", "L1": "scaled", "L2": "scaled"}),
        ]
        cumulative = lm._build_cumulative(months)
        self.assertEqual(cumulative["months_included"], 2)
        self.assertTrue(cumulative["includes_scaled_months"])
...
class BuildInProgressTest(unittest.TestCase):
    """オーナー承認機能（2026-09-06、月途中集計）。非連続usable日集合への変更は
    オーナー決定2026-09-20（S_1深放電インシデント対策 #5・Pi Wi-Fi断でin_progressが
    恒久停止した実害への対応）。"""
    ...
    def test_period_end_actual_is_today_minus_one_regardless_of_gaps(self):
        # オーナー決定2026-09-20: period_end_actual はusable日の有無に関わらず
        # min(end, today-1日)に固定する（従来の「startから連続してusableな最終日で打ち切る」
        # 動作は、窓の途中の1日欠測でそれ以降のusable日を全て捨ててしまい、in_progressが
        # 恒久的に止まる実害があったため廃止した）。
        tariff = make_tariff()
        ...
        self.assertEqual(result["period_end_actual"], "2026-09-19")  # today(09-20)の前日
        self.assertEqual(result["days_elapsed"], 18)  # 09-02〜09-19の暦日数
        self.assertEqual(len(result["excluded_dates"]), 16)  # 09-04〜09-19
        for key in ("L0", "L1", "L2", "L3"):
    ...
    def test_includes_usable_day_after_mid_window_profile_gap(self):
        # オーナー決定2026-09-20: 窓の途中(09-03)にprofile欠測日が挟まっていても、
        # それ以降の09-04がusable（profile+daily.json両方あり）なら中抜けのまま含める
        # （連続していなくてよい）。09-03は excluded_dates に理由付きで記録される。
        tariff = make_tariff()
        profile_by_date = {
            "2026-09-02": build_synthetic_golden_day_buckets(day="2026-09-02"),
        ...
        result = lm.build_in_progress(tariff, daily_by_date, profile_by_date, today=date(2026, 9, 20))
        self.assertIsNotNone(result)
        self.assertEqual(result["days_covered"], 2)  # 09-02, 09-04（09-03は非連続だが含めない）
        self.assertEqual(result["layers"]["L3"]["buy_kwh"], 4.0)  # 1.0(09-02) + 3.0(09-04)
        self.assertTrue(result["layers"]["L0"]["available"])
        excluded_09_03 = next(e for e in result["excluded_dates"] if e["date"] == "2026-09-03")
        self.assertEqual(excluded_09_03["reason_code"], "profile_missing")
        self.assertIn("reason_label", excluded_09_03)
    ...
    def test_in_progress_layers_cover_identical_day_span(self):
        # QA再レビュー #1（窓ずれ修正の不変条件は維持）: L0〜L2はprofileがusableな日のみ、
        # L3はdaily.jsonにある日のみを別々に合算すると両者の日集合がずれる
        # （実データでL0=3日、L3=4日というQA指摘事例があった）。修正後はprofile-usable
        # と daily.json-present の共通集合（usable_dates）を4層すべてで使うことを確認する。
        tariff = make_tariff()
        profile_by_date = {
            "2026-09-02": build_synthetic_golden_day_buckets(day="2026-09-02"),
            "2026-09-03": build_synthetic_golden_day_buckets(day="2026-09-03"),
        ...
        self.assertEqual(result["period_end_actual"], "2026-09-19")  # today(09-20)の前日
        self.assertEqual(result["days_covered"], 2)
        # L3もL0〜L2と同じ09-02〜09-03の2日分(buy_kwh=2.0)だけを合算する（09-04,09-05は
        # daily.jsonにはあるがprofileがusableでないため両方から除外される）。
        self.assertEqual(result["layers"]["L3"]["buy_kwh"], 2.0)
        self.assertTrue(result["layers"]["L0"]["available"])
        excluded_dates_set = {e["date"] for e in result["excluded_dates"]}
        self.assertIn("2026-09-04", excluded_dates_set)
        self.assertIn("2026-09-05", excluded_dates_set)

    def test_excluded_dates_have_daily_missing_reason_when_profile_ok_but_sensor_missing(self):
        # excluded_dates の理由コードが profile_missing/period_incomplete/daily_missing で
        # 使い分けられることを確認する（profile usableだがdaily.jsonにその日が無いケース）。
        tariff = make_tariff()
        profile_by_date = {
            "2026-09-02": build_synthetic_golden_day_buckets(day="2026-09-02"),
            "2026-09-03": build_synthetic_golden_day_buckets(day="2026-09-03"),
        ...
        daily_by_date = {
            "2026-09-02": {"date": "2026-09-02", "buy_kwh": 1.0, "sell_kwh": 0.5},
            # 09-03はprofileはusableだがdaily.json側に無い
        }
        result = lm.build_in_progress(tariff, daily_by_date, profile_by_date, today=date(2026, 9, 20))
        self.assertIsNotNone(result)
        self.assertEqual(result["days_covered"], 1)
        excluded_09_03 = next(e for e in result["excluded_dates"] if e["date"] == "2026-09-03")
        self.assertEqual(excluded_09_03["reason_code"], "daily_missing")
...
class ResolveDayBucketsTest(unittest.TestCase):
    """オーナー承認機能（2026-09-06、QA再レビュー同日、閾値緩和オーナー決定2026-09-20）:
    1日INTERPOLATION_MAX_GAP(6)バケットまでの欠落は線形補間してusable にする（DDR §0既知の
    ノイズ対策。nichicon_realtime_historyは300秒瞬時値でジッタが常態）。resolve_day_buckets()
    は DayResolution(buckets, reason_detail, interpolated_values, interpolated_slots) を返す。"""
    ...
    def test_three_gaps_in_one_channel_is_still_usable(self):
        # 補間許容の範囲内(INTERPOLATION_MAX_GAP=6)にある欠落は補間される非境界のケース。
        # 境界値そのものは test_six_gaps_in_one_channel_is_still_usable /
        # test_seven_gaps_in_one_channel_remains_unusable を参照。
        buckets = self._golden_day()
    ...
    def test_six_gaps_in_one_channel_is_still_usable(self):
        # オーナー決定2026-09-20（Pi Wi-Fi断で1日4〜6スロット欠落した実害への対応）: 境界値
        # テスト。INTERPOLATION_MAX_GAP(6)ちょうどの欠落は補間される。INTERPOLATION_MAX_GAPを
        # 5に変更すると本テストは失敗する（境界を検出するための設計）。
        buckets = self._golden_day()
        for i in (100, 101, 102, 103, 104, 105):
            buckets[i] = make_bucket(buckets[i].bucket_at, eco_ac_in_w=None)
        r = lm.resolve_day_buckets(buckets, date(2026, 9, 1))
        self.assertIsNotNone(r.buckets)
        self.assertEqual(r.interpolated_values, 6)
        self.assertEqual(r.interpolated_slots, 6)
        self.assertLessEqual(6, lm.INTERPOLATION_MAX_GAP)  # 境界値であることの前提を明記

    def test_seven_gaps_in_one_channel_remains_unusable(self):
        buckets = self._golden_day()
    ...
    def test_soc_gap_beyond_limit_makes_day_unusable(self):
        # QA再レビュー #4: nichicon_socの欠落もmax_gapに含める。socだけ7個欠落していても不採用
        # （オーナー決定2026-09-20でINTERPOLATION_MAX_GAPが3→6に緩和されたため境界を7個に更新）。
        buckets = self._golden_day()
...
class QualityGateTest(unittest.TestCase):
    ...
    def test_day_with_many_null_field_buckets_is_unusable(self):
        # オーナー決定2026-09-20: 欠落7個は補間の許容(INTERPOLATION_MAX_GAP=6)を超えるため
        # 不採用のまま。
        buckets = build_synthetic_golden_day_buckets(day="2026-09-01")
        for i in (10, 11, 12, 13, 14, 15, 16):
            buckets[i] = make_bucket(buckets[i].bucket_at, eco_ac_in_w=None)
    ...
    def test_partial_month_marks_layer_unavailable(self):
        # オーナー決定2026-09-20: usable日が請求期間の90%未満ならL0〜L2はunavailable。
        # 31日中4日欠測(27/31≈87.1%<90%)で不可（90%以上のケースはBuildMonthLayersScalingTest参照）。
        tariff = make_tariff()
        start, end = bill_model.billing_period("2026-09", meter_read_day=2)
        daily = {}
        d = start
        ...
        missing = {"2026-08-05", "2026-08-12", "2026-08-19", "2026-08-31"}
        profile_by_date = {}
        d = start
        while d <= end:
        ...
        self.assertFalse(record["layers"]["L0"]["available"])
        l0_reason = record["layers"]["L0"]["unavailable_reason"]
        # 4日欠測(31日中)なので profile_missing ではなく period_incomplete。
        self.assertEqual(l0_reason["reason_code"], "period_incomplete")
        self.assertEqual(l0_reason["reason_label"], "5分プロファイル欠落（4日）")
        self.assertIn("5分プロファイル欠測", l0_reason["reason_detail"])
        self.assertIn("4/31", l0_reason["reason_detail"])
    ...
    def test_full_month_available_when_all_days_usable(self):
        # coverage==1.0（全日usable）ならL0〜L2はavailableになり、estimation:"full"
        # （スケールなし、オーナー決定2026-09-20）になる。
        tariff = make_tariff()
        start, end = bill_model.billing_period("2026-09", meter_read_day=2)
        daily = {}
        profile_by_date = {}
        ...
        self.assertEqual(record["layers"]["L0"]["estimation"], "full")
        self.assertEqual(record["layers"]["L0"]["days_usable"], 31)
        self.assertEqual(record["layers"]["L0"]["days_total"], 31)
        self.assertEqual(record["layers"]["L0"]["scale_factor"], 1.0)
        self.assertEqual(record["layers"]["L0"]["excluded_dates"], [])

    def test_month_with_small_gap_is_still_available_via_interpolation(self):
        # オーナー承認機能（2026-09-06）: 1日INTERPOLATION_MAX_GAPバケットまでの欠落は
        # 線形補間されるため、coverage==1.0のまま available になり、interpolated_buckets
        # に補間数が記録される。
        tariff = make_tariff()
        start, end = bill_model.billing_period("2026-09", meter_read_day=2)
        daily = {}
...
class BuildMonthLayersScalingTest(unittest.TestCase):
    """オーナー決定2026-09-20: usable日が請求期間の90%以上100%未満の確定月は、usableな日
    （連続でなくてよい）のみで求めたbuy_kwh/sell_kwhを「期間日数／usable日数」で月額換算
    (scale_factor)してからcompute_billに渡す（estimation:"scaled"）。100%は従来どおり
    スケールなし(estimation:"full")、90%未満はunavailable（境界値そのものはQualityGateTest.
    test_partial_month_marks_layer_unavailable / test_full_month_available_when_all_days_usable
    を参照）。L3（実測）はこのスケーリングと無関係（請求実績・センサー日次をそのまま使う）。"""

    def _profile_with_missing(self, start, end, missing: set) -> dict:
        profile_by_date = {}
        d = start
        while d <= end:
            if d.isoformat() not in missing:
    ...
    def test_scaled_month_l0_buy_matches_full_month_equivalent_and_l3_stays_billed(self):
        # 31日中2日欠測(29/31≈93.5%、90%以上100%未満なのでscaled)。合成golden dayは全日
        # 同一プロファイルのため、scale_factor(31/29)を掛けたL0買電は「全31日そろっていた
        # 場合」と厳密に一致するはず（日数比換算の正しさをこの性質で検証する）。
        tariff = make_tariff()
        start, end = bill_model.billing_period("2026-09", meter_read_day=2)
        daily = {}
        d = start
        ...
        official_buy_by_month = {
            "2026-09": {
                "settlement_month": "2026-09", "period_from": start.isoformat(), "period_to": end.isoformat(),
                "official_buy_kwh": 31.0, "billed_yen": 9999,
            }
        }

        missing = {"2026-08-09", "2026-08-23"}
        profile_by_date = self._profile_with_missing(start, end, missing)
        full_profile_by_date = self._profile_with_missing(start, end, set())

        record = lm.build_month_layers(tariff, "2026-09", daily, {}, official_buy_by_month, profile_by_date)
        full_record = lm.build_month_layers(tariff, "2026-09", daily, {}, official_buy_by_month, full_profile_by_date)

        l0 = record["layers"]["L0"]
        self.assertTrue(l0["available"])
        self.assertEqual(l0["estimation"], "scaled")
        self.assertEqual(l0["days_usable"], 29)
        self.assertEqual(l0["days_total"], 31)
        self.assertAlmostEqual(l0["scale_factor"], 31 / 29, places=3)
        self.assertEqual(sorted(e["date"] for e in l0["excluded_dates"]), sorted(missing))
        for e in l0["excluded_dates"]:
            # profile_by_date にその日のキー自体が無い（未取得）ケースなので profile_missing。
            self.assertEqual(e["reason_code"], "profile_missing")

        full_l0 = full_record["layers"]["L0"]
        self.assertEqual(full_l0["estimation"], "full")
        self.assertAlmostEqual(l0["buy_kwh"], full_l0["buy_kwh"], places=2)
        self.assertAlmostEqual(l0["net_cost_fit_yen"], full_l0["net_cost_fit_yen"], delta=2)

        # L3は請求実績(billed)をそのまま使い、profile欠測やscale_factorの影響を受けない
        # （非対称の開示: L0〜L2は日数比換算の推定、L3は実請求）。
        l3 = record["layers"]["L3"]
        self.assertEqual(l3["buy_source"], "billed")
        self.assertEqual(l3["buy_kwh"], 31.0)

    def test_month_below_90_percent_usable_is_unavailable(self):
        tariff = make_tariff()
        start, end = bill_model.billing_period("2026-09", meter_read_day=2)
        daily = {}
        d = start
        while d <= end:
            daily[d.isoformat()] = {"date": d.isoformat(), "buy_kwh": 1.0, "solar_kwh": 10.0, "sell_kwh": 5.0}
            d += timedelta(days=1)
        missing = {"2026-08-03", "2026-08-10", "2026-08-17", "2026-08-24"}  # 27/31≈87.1%<90%
        profile_by_date = self._profile_with_missing(start, end, missing)
        record = lm.build_month_layers(tariff, "2026-09", daily, {}, {}, profile_by_date)
        self.assertFalse(record["layers"]["L0"]["available"])
        self.assertNotIn("estimation", record["layers"]["L0"])

    def test_scaled_month_does_not_fall_back_to_provisional_tariff(self):
        # 暫定単価を確定月に使わないルールは維持する（build_in_progressのみが
        # resolve_effective_tariffフォールバックを許容し、確定月のscaled/full経路では
        # 直近確定月への暫定フォールバックを一切使わない）。
        tariff = make_tariff(
            fuel_cost_adjustment_yen_per_kwh={"2026-08": -3.50},
            capacity_contribution_yen_per_month={"2026-08": 213},
        )  # 2026-09は未確定、2026-08だけ確定（暫定フォールバック元が実在する状態）
        start, end = bill_model.billing_period("2026-09", meter_read_day=2)
        missing = {"2026-08-09", "2026-08-23"}  # 29/31≈93.5%（scaled相当のusable率）
        profile_by_date = self._profile_with_missing(start, end, missing)
        record = lm.build_month_layers(tariff, "2026-09", {}, {}, {}, profile_by_date)
        for key in ("L0", "L1", "L2"):
            self.assertFalse(record["layers"][key]["available"], f"{key} should be unavailable")
